...
- Test, lint, and typecheck when you expect them green — `cmd1 && cmd2 && cmd3`, one call.
  Split them only after something fails and you need to isolate which.
...
5. **Verify.**
   ...
   - `ci: run` → watch CI with a **single blocking call**: `gh run watch <id>
     --exit-status` on GitHub, or the shell poll loop in
     [../references/forge.md](../references/forge.md) on Gitea. **Never take an agent turn
     per status check** — one turn per poll re-reads your entire context, so a 20-minute
     CI run costs 40 full-context round trips instead of one. On failure, read failing logs
...
**At ~120 tool calls, stop at the next clean point** and return `outcome: "checkpoint"`.
...
Checkpoint *early*, before the budget, when:
