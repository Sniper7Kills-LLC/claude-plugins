...
4. **Review cycle.** Regenerate the rendered site (`python3
   docs/specs/render-spec.py`), open it, and run the same approve/revise loop as
   planner Phase 4 — including collecting `docs/specs/review-comments.md` if the user
   commented in the rendered pages, acting on each open comment and marking it
   resolved. A spec that was `approved` returns to `approved` only through the user's
   explicit approval of this round; record the round in `## Changelog`.
...
1. **Diff the package against the current contracts** —
   ...
   - Single-page renderer (plugin ≤ 0.14.0): replace the project's copied
     `render-spec.py` with the skill's current one and regenerate — the render is now
     multi-page (`spec.html` index + `html/` leaf pages, `pages:` front-matter
     supported, in-page review layer). Commit `html/` and, when present,
     `review-comments.md` with the package; never leave them untracked.
...
- [ ] `spec.html` + `html/` regenerated by `render-spec.py` after the last edit;
      `html_generated` stamped; any `review-comments.md` round fully resolved and
      committed.
