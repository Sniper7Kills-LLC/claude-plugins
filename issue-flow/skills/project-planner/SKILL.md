...
4. On revise: edit the spec files and mockups, **rerun `python3
   docs/specs/render-spec.py`**, re-open, and re-present. Loop until approved. Record
   every round in `## Changelog`. Never edit `spec.html` by hand — the renderer
   overwrites it.
5. On approval:
...
- [ ] `docs/specs/mockups/*.html` — self-contained, each referenced by exactly one
      ...
      regenerated.
- [ ] `docs/specs/spec.html` — generated by `docs/specs/render-spec.py` after the last
      edit (never hand-written), `html_generated` set, the full spec and feature content
      rendered in place, diagrams rendering via the vendored
