...
├── spec.html               # generated site index (Phase 3)
├── html/                   # generated: one page per feature and supplement page
├── review-comments.md      # exported review feedback, when a round produced any
├── features/
│   ├── 01-<feature>.md     # one file per feature set — the deep detail
│   └── ...
├── pages/                  # optional supplement pages (implementation plan, infra…)
└── mockups/
...
Generate `spec.html` from the markdown — copy the skill's `scripts/render-spec.py`
into `docs/specs/`, vendor the mermaid asset, and run it; the markdown is the spec and
`spec.html` is its generated website, never a hand-written summary. Then `CLAUDE.md`,
`docs/adr/` + `docs/external.md`,
...
3. Ask via `AskUserQuestion`: **Approve** / **Revise** (collect what to change), plus
   the specific items from `Risks & open questions`. Tell the user they can also
   comment directly in the rendered pages (hover a heading, or the Review drawer) and
   **Export** the result as `docs/specs/review-comments.md` — feedback lands next to
   the section it is about instead of retyped in chat.
4. On revise: read `docs/specs/review-comments.md` if it exists and act on every
   `status: open` comment, flipping each to `status: resolved` in place (the format
   and loop are in `references/spec-format.md` § Review comments); apply the chat
   feedback; edit the spec files and mockups; **rerun `python3
   docs/specs/render-spec.py`**, re-open, and re-present — the render embeds every
   comment under its section so the user sees each one answered in context. Loop
   until approved. Record every round in `## Changelog`. Never edit `spec.html` or
   `html/` by hand — the renderer overwrites them.
5. On approval:
...
- [ ] `docs/specs/features/*.md` — one per feature set, each with a stable `id:`,
      ...
      numbered, singular and testable, and covered by acceptance criteria; every user
      flow opens with its mermaid diagram and the diagram agrees with the steps.
- [ ] `docs/specs/mockups/*.html` — self-contained, each referenced by exactly one
      ...
      regenerated.
- [ ] `docs/specs/spec.html` + `docs/specs/html/` — generated by
      `docs/specs/render-spec.py` after the last edit (never hand-written),
      `html_generated` set, the full spec, every feature and every `pages:` entry
      rendered in full on its own page, diagrams rendering via the vendored
      `docs/specs/assets/mermaid.min.js` (or the vendoring failure reported), every
      mockup embedded with both an iframe and a visible fallback link, all links
      relative, no stale pages left in `html/`.
- [ ] `docs/specs/review-comments.md` — if any review round produced one, every
      comment is `status: resolved` (or explicitly left open with the user's consent)
      and the file is committed with the spec.
