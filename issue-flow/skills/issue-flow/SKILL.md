...
- **`ready-to-merge`** → sub-merge below (never on the worker's word alone):
  ...
  4b. **Check what actually landed, and re-trigger if the batch PR is already open.** The merged-into branch is the only place that shows the real message, so read it there:
     ...
     Then: **if the batch PR for this integration branch is already open, push a fresh trigger commit now** — subject only, no body, verified with the pre-push check in C2 step 1 — and re-anchor the CI watch to the new head SHA. This step exists because the suppression happens *here*, in C1, while the rule about re-triggering lives in C2 step 1, and worker completions arrive asynchronously long after C2 step 1 was "done". A member that sub-merges after the batch PR opened leaves the batch gated on a run for a commit that is no longer head; on GitHub it leaves no run at all.
...
1. Open **one PR: integration branch → dev.** Title `Epic #<n>: <title>` / `Batch #<n>: <summary>`; body lists every member with `Closes #<m>` lines. Push an empty commit **without** the skip token to make CI run (`git commit --allow-empty -m "ci: run full suite for batch #<n>"` — one `-m`, subject only, **no body**).
   ...
   **On `no-run-registered`, read the commit before reacting** — `git log -1 --format='%s%n%b' <sha> | grep -niE '\[(skip[ -]?ci|ci skip|no ci|skip actions|actions skip)\]' || true` (the `|| true` matters: `grep` exits 1 on zero matches, which is the *no-token* branch — the one that must still be reachable inside a `set -e` script). A hit is a suppressed commit: push one clean subject-only trigger and re-watch. No hit is a runner or workflow-file problem, which a re-push does not fix — resolve it or substitute the local gate at step 3 and say so on the PR. One clean re-trigger, not a loop.
